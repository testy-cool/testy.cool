...
function parseBody(value: unknown): string {
  if (typeof value !== "string") {
    throw new Error("Thought must be text");
  }

  const body = value.trim();
  if (!body) throw new Error("Write something first");
  if (body.length > MAX_BODY_LENGTH) {
    throw new Error(`Thoughts can be at most ${MAX_BODY_LENGTH.toLocaleString("en-US")} characters`);
  }
...
function parseStatus(value: unknown): PostStatus {
  if (value !== "thought" && value !== "idea") {
    throw new Error("Status must be thought or idea");
  }
...
function normalizeTags(values: unknown[]): string[] {
  ...
  if (unique.length > MAX_TAGS) throw new Error(`Use at most ${MAX_TAGS} tags`);
  if (unique.some((tag) => tag.length > MAX_TAG_LENGTH)) {
    throw new Error(`Tags can be at most ${MAX_TAG_LENGTH} characters`);
  }
...
export function parsePostId(value: string): number {
  if (!/^\d+$/.test(value)) throw new Error("Invalid post id");
  const id = Number(value);
  if (!Number.isSafeInteger(id) || id < 1) throw new Error("Invalid post id");
  return id;
}
