...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  if (channelId) {
    ...
    return new Response(
      JSON.stringify({ result: raw ? JSON.parse(raw) : null }),
      {
        headers: { "Content-Type": "application/json" },
      },
    );
  ...
  return new Response(
    JSON.stringify({ error: "Missing action or channelId" }),
    { status: 400 },
  );
};
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  if (!channelId || !result) {
    return new Response(
      JSON.stringify({ error: "Missing channelId or result" }),
      { status: 400 },
    );
  }

  // Save the full result
  await kv.put(`channel:${channelId}`, JSON.stringify(result), {
  ...
  const updated = [
    summary,
    ...recent.filter((c) => c.channelId !== channelId),
  ].slice(0, MAX_RECENT);
