/**
 * Cloudflare Pages Function that intercepts /tools/video-breakdown requests.
...
function buildRichMetaTags(tutorial: TutorialCache): string {
  const title = escapeHtml(`${tutorial.title} — Video Breakdown`);
    ...
    "@graph": [
      {
        ...
        description: `${tutorial.steps.length}-chapter video breakdown generated from "${tutorial.videoTitle}"`,
